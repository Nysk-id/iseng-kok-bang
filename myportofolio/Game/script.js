...
$(function () {
    ...
    */
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            /*
             $.post('http://hagg-lockpick/exit', JSON.stringify({
                 success: false
             }));
             */
...
function outOfPins() {
    /*
    gameOver = true;
    $.post('http://hagg-lockpick/callback', JSON.stringify({
        success: false
    }));
    */
    setTimeout(function(){
    ...
    alert("Kamu Gagal, lockpik kamu hancur dan Silahkan Ulangi Lagi");
    gameOver = false
    gamePaused = false
}

function unlock() {
    alert("Selamat " + nama +", Anda Berhasil membuka kunci");
    gameOver = false
    gamePaused = false
